...
| UI | 自研**苹果风格设计系统**（`src/styles/global.css`）+ 原生组件 | 毛玻璃侧栏、圆角卡片、SF 风格 SVG 图标、交通灯窗口 |
...
├─ src/                                             # 前端 React（苹果风格 UI）
│  ├─ main.tsx                # 入口
│  ├─ App.tsx                 # 应用骨架：侧栏 + 顶栏(交通灯) + 内容 + 右栏 + 任务栏
│  ├─ styles/global.css       # 苹果风格设计系统（CSS 变量 / 毛玻璃 / 圆角 / 交通灯）
...
│  ├─ components/icons.tsx    # 模块 SVG 图标
│  └─ modules/
...
_由 大幂幂 规划整理 · 2026-07-10（三大模块优化重组版 · 在原始三大需求基础上演进；同日落地 Tauri2 + React + TS 工程骨架与苹果风格 UI）_
